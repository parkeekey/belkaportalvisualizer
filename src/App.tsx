...
function App() {
  ...
  const [darkMode, setDarkMode] = useState(() => {
    try {
      const saved = localStorage.getItem('belka.darkMode');
      return saved === 'true';
    } catch {
      return false;
    }
  ...
  useEffect(() => {
    try {
      localStorage.setItem('belka.darkMode', String(darkMode));
      if (darkMode) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-950">
          ...
          <div className="flex flex-wrap justify-between items-center gap-y-2 py-3">
            ...
            <div className="flex flex-wrap items-center gap-1 sm:gap-2">
              ...
              <button
                ...
                className={`px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm font-semibold rounded-lg border transition-colors ${activePage === 'digitizer' ? 'bg-slate-800 dark:bg-slate-700 border-slate-800 dark:border-slate-600 text-white' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700'}`}
              ...
              <button
                ...
                className={`px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm font-semibold rounded-lg border transition-colors ${activePage === 'ultrakoki-parser' ? 'bg-emerald-600 dark:bg-emerald-700 border-emerald-600 dark:border-emerald-600 text-white' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700'}`}
              ...
              <button
                ...
                className={`px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm font-semibold rounded-lg border transition-colors ${activePage === 'setup-profile' ? 'bg-sky-600 dark:bg-sky-700 border-sky-600 dark:border-sky-600 text-white' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700'}`}
              ...
              <button
                ...
                className="flex items-center gap-1 px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm font-semibold rounded-lg border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
                title="About this app — EC, TDS, use cases, credits"
              >
                <span className="inline-flex items-center justify-center w-3.5 h-3.5 sm:w-4 sm:h-4 rounded-full bg-slate-500 dark:bg-slate-600 text-white text-[8px] sm:text-[10px] font-bold leading-none">i</span>
                About
              </button>
              <button
                onClick={() => setDarkMode(!darkMode)}
                className={`px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm font-semibold rounded-lg border transition-colors ml-2 ${
                  darkMode
              ...
              <button
                ...
                className={`px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm font-semibold rounded-lg border transition-colors ${
                  showProfileBar
                    ? 'border-sky-300 dark:border-sky-600 bg-sky-100 dark:bg-sky-900 text-sky-900 dark:text-sky-100'
      ...
      {showProfileBar && (
        <section className="border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900/95">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <div className="text-sm font-semibold text-slate-900 dark:text-slate-100">Workspace Profiles</div>
                <p className="text-xs text-slate-500 dark:text-slate-400">Save the full main app + parser session into one portable file, or restore it later.</p>
              </div>
              <div className="flex flex-wrap items-center gap-2">
                <button
                  onClick={saveWorkspaceProfile}
                  className="px-3 py-2 text-sm font-semibold rounded-lg border border-sky-200 dark:border-sky-700 bg-sky-50 dark:bg-sky-900 text-sky-800 dark:text-sky-200 hover:bg-sky-100 dark:hover:bg-sky-800"
                ...
                <button
                  ...
                  className="px-3 py-2 text-sm font-semibold rounded-lg border border-amber-200 dark:border-amber-700 bg-amber-50 dark:bg-amber-900 text-amber-900 dark:text-amber-200 hover:bg-amber-100 dark:hover:bg-amber-800"
