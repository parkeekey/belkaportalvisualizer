  ...
  return (
    <div ref={ref} className="relative">
      <button type="button" onClick={() => setOpen(p => !p)}
        className={`w-full ${cls.input} border rounded font-mono focus:outline-none focus:ring-1 focus:ring-amber-400 text-left flex items-center gap-1.5 ${value ? 'bg-white border-slate-300' : 'bg-white border-slate-200 text-slate-400'}`}
      >
        {value ? (
          <>
          ...
          <span className="text-slate-400">{placeholder ?? 'Origin'}</span>
        )}
      ...
      {open && (
        <div className="absolute z-20 top-full mt-1 left-0 right-0 bg-white border border-slate-200 rounded-lg shadow-lg max-h-56 overflow-y-auto">
          <div className="sticky top-0 bg-white border-b border-slate-100 p-1">
            <input ref={inputRef} value={query} onChange={e => setQuery(e.target.value)}
              placeholder="Search origins..."
              className={`w-full ${cls.input} border border-slate-200 rounded font-mono focus:outline-none focus:ring-1 focus:ring-amber-400 bg-slate-50`}
            />
          </div>
          <div className="py-1">
            {value && (
              <button onClick={() => { onChange(''); setOpen(false); setQuery(''); }}
                className={`w-full text-left px-2 py-1 ${cls.dropdown} text-slate-400 hover:bg-slate-50 italic`}
              >— Clear —</button>
            )}
            {results.length === 0 ? (
              <div className={`px-2 py-2 ${cls.dropdown} text-slate-400 italic text-center`}>No matches</div>
            ) : (() => {
                  ...
                  <div key={region}>
                    ...
                    {origins.map(o => (
                      <button key={o.name} onClick={() => { onChange(o.name); setOpen(false); }}
                        className={`w-full text-left px-2 py-1 ${cls.dropdown} hover:bg-amber-50 flex items-center gap-1.5 ${value === o.name ? 'bg-amber-50 font-semibold text-amber-800' : 'text-slate-600'}`}
                      >
                        <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ backgroundColor: rc?.color }} />
                        <span>{o.name}</span>
                        {o.subregion && <span className="text-slate-300 ml-auto text-[0.8em]">{o.subregion}</span>}
                      </button>
                    ))}
                  </div>
                );
