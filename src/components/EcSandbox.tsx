...
export default function EcSandbox({
  ...
  return (
    <div className={`border-t pt-3 mt-3 ${isDemo ? 'border-dashed border-slate-300 dark:border-slate-600' : 'border-slate-200 dark:border-slate-700'}`}>
      <div className="flex items-center justify-between mb-2">
        <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">EC Dial-In Sandbox</span>
        <div className="flex items-center gap-2">
          {isDemo && (
            <span className="text-[7px] font-bold text-emerald-600 uppercase tracking-wider bg-emerald-50 dark:bg-emerald-900/20 px-1 py-0.5 rounded border border-emerald-300 dark:border-emerald-700">Demo live data</span>
          )}
          <button onClick={onToggle}
            className="text-[8px] text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:text-slate-400 underline decoration-dotted"
          >✕ Disable</button>
        </div>
      </div>

      {isDemo && (
        <p className="text-[8px] text-slate-500 dark:text-slate-400 italic mb-2">
      ...
      {analysis && (
        <div className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-4 gap-2 text-[9px]">
            <div className="bg-slate-50 dark:bg-slate-900/50 rounded p-1.5 text-center">
              <div className="text-slate-400 dark:text-slate-500">Lowest EC</div>
              <div className="font-bold text-red-600">{analysis.lowestEC.toFixed(1)}</div>
              <div className="text-slate-400 dark:text-slate-500">@{Math.floor(analysis.timeToLowestEC)}s</div>
            </div>
            <div className="bg-slate-50 dark:bg-slate-900/50 rounded p-1.5 text-center">
              <div className="text-slate-400 dark:text-slate-500">Peak EC</div>
              <div className="font-bold text-emerald-600">{analysis.highestEC.toFixed(1)}</div>
              <div className="text-slate-400 dark:text-slate-500">@{Math.floor(analysis.timeToHighestEC)}s</div>
            </div>
            <div className="bg-slate-50 dark:bg-slate-900/50 rounded p-1.5 text-center">
              <div className="text-slate-400 dark:text-slate-500">Red light cross</div>
              <div className={`font-bold ${analysis.timeBelowRedLight !== null ? 'text-amber-600 dark:text-amber-400' : 'text-slate-400 dark:text-slate-500'}`}>
                {analysis.timeBelowRedLight !== null ? `${Math.floor(analysis.timeBelowRedLight)}s` : 'Never'}
              </div>
              <div className="text-slate-400 dark:text-slate-500">EC &lt; {redLightThreshold.toFixed(1)}</div>
            </div>
            <div className="bg-slate-50 dark:bg-slate-900/50 rounded p-1.5 text-center">
              <div className="text-slate-400 dark:text-slate-500">Shape</div>
              <div className={`font-bold ${
                analysis.shape === 'stable' ? 'text-emerald-600' :
                analysis.shape === 'stalling' ? 'text-amber-600 dark:text-amber-400' :
                analysis.shape === 'collapsed' ? 'text-red-600' : 'text-slate-500 dark:text-slate-400'
              }`}>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-[9px] text-slate-400 dark:text-slate-500 font-semibold">Grind size:</span>
            ...
            />
            <span className="text-[9px] text-slate-400 dark:text-slate-500">µm</span>
            <input value={grindUm ?? ''}
              onChange={e => {
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-[9px] text-slate-400 dark:text-slate-500 font-semibold">Target finish EC:</span>
          ...
          <div className="flex items-center gap-1 flex-wrap">
            <span className="text-[9px] text-slate-400 dark:text-slate-500 font-semibold">Actual finish time:</span>
            <input value={Math.floor(actualFinishTimeSec / 60)}
              ...
              className="w-10 px-1 py-0.5 text-[10px] border border-slate-200 dark:border-slate-700 rounded font-mono text-center focus:outline-none focus:ring-1 focus:ring-slate-400"
              placeholder="m"
            />
            ...
            {ecAtFinishTime !== null && (
              <span className="text-[9px] text-slate-500 dark:text-slate-400">
                EC @ finish: <strong className="text-slate-700 dark:text-slate-300">{ecAtFinishTime.toFixed(1)}</strong>
              </span>
            )}
            {hasTargetEC && timeDelta !== null && (
              <span className={`text-[9px] ${timeDelta > 10 ? 'text-amber-600 dark:text-amber-400' : timeDelta < -10 ? 'text-red-600' : 'text-emerald-600'}`}>
                {timeDelta > 10
          ...
          {rec && (
            <div className={`rounded-lg border p-2.5 ${
              rec.verdict === 'In the zone' ? 'bg-emerald-50 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-800' :
              rec.verdict === 'Bed collapsed' ? 'bg-red-50 dark:bg-red-900/20 border-red-200' :
              rec.verdict === 'Bed stalling' ? 'bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800' :
              rec.verdict === 'Channeling detected' ? 'bg-orange-50 border-orange-200' :
              'bg-slate-50 dark:bg-slate-900/50 border-slate-200 dark:border-slate-700'
            }`}>
              <div className="flex items-center justify-between mb-1">
                <span className={`text-[11px] font-bold ${
                  rec.verdict === 'In the zone' ? 'text-emerald-700 dark:text-emerald-400' :
                ...
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${
                  rec.action === 'No changes needed' ? 'bg-emerald-200 text-emerald-800 dark:text-emerald-200' :
                  rec.action.startsWith('Grind finer') ? 'bg-red-200 text-red-800' :
                  rec.action.startsWith('Grind coarser') ? 'bg-amber-200 text-amber-800 dark:text-amber-200' :
              ...
              <div className="grid grid-cols-3 gap-1.5 mb-1.5">
                <div className="bg-white dark:bg-slate-800/60 rounded px-1.5 py-1 text-center">
                  <div className="text-[6px] text-slate-400 dark:text-slate-500 uppercase tracking-wider">EC low</div>
                  <div className="text-[10px] font-bold text-red-600">{analysis.lowestEC.toFixed(1)}</div>
                  <div className="text-[7px] text-slate-400 dark:text-slate-500">@{Math.floor(analysis.timeToLowestEC)}s</div>
                </div>
                <div className="bg-white dark:bg-slate-800/60 rounded px-1.5 py-1 text-center">
                  <div className="text-[6px] text-slate-400 dark:text-slate-500 uppercase tracking-wider">EC peak</div>
                  <div className="text-[10px] font-bold text-emerald-600">{analysis.highestEC.toFixed(1)}</div>
                  <div className="text-[7px] text-slate-400 dark:text-slate-500">@{Math.floor(analysis.timeToHighestEC)}s</div>
                </div>
                <div className="bg-white dark:bg-slate-800/60 rounded px-1.5 py-1 text-center">
                  <div className="text-[6px] text-slate-400 dark:text-slate-500 uppercase tracking-wider">Red light</div>
                  <div className={`text-[10px] font-bold ${analysis.timeBelowRedLight !== null ? 'text-amber-600 dark:text-amber-400' : 'text-slate-400 dark:text-slate-500'}`}>
                    {analysis.timeBelowRedLight !== null ? `${Math.floor(analysis.timeBelowRedLight)}s` : '—'}
                  </div>
                  <div className="text-[7px] text-slate-400 dark:text-slate-500">EC &lt; {redLightThreshold.toFixed(1)}</div>
                </div>
              </div>

              {/* Explanation — one line */}
              <div className="text-[8px] text-slate-500 dark:text-slate-400 leading-relaxed mb-1.5">
                {(() => {
                  switch (analysis.shape) {
              ...
              {rec.suggestedGrindDelta !== 0 && grindUm && (
                <div className="bg-white dark:bg-slate-800/60 rounded px-2 py-1.5 mb-1.5 border border-slate-200 dark:border-slate-700/50">
                  <div className="text-[7px] text-slate-400 dark:text-slate-500 uppercase tracking-wider font-semibold mb-1">Grind suggestion</div>
                  <div className="grid grid-cols-[auto_1fr_auto_1fr] gap-x-2 gap-y-0.5 text-[9px] items-center">
                    ...
                    <span className="font-bold text-slate-700 dark:text-slate-300">{grindInput || '—'} <span className="font-normal text-slate-400 dark:text-slate-500 text-[8px]">clicks</span></span>
                    <span className="font-bold text-slate-700 dark:text-slate-300 text-right">{grindUm}µm</span>
                    <span className="text-slate-300 dark:text-slate-600 text-center text-[8px]">│</span>

                    <span className="text-slate-400 dark:text-slate-500">Suggested:</span>
                    <span className="font-bold text-slate-700 dark:text-slate-300">
                      ...
                      })()} <span className="font-normal text-slate-400 dark:text-slate-500 text-[8px]">clicks</span>
                    </span>
                    <span className={`font-bold text-right ${rec.suggestedGrindDelta < 0 ? 'text-red-600' : 'text-amber-600 dark:text-amber-400'}`}>
                      ~{Math.max(300, Math.min(1800, grindUm + rec.suggestedGrindDelta))}µm
                    </span>
                    <span className={`text-[8px] text-right ${rec.suggestedGrindDelta < 0 ? 'text-red-500 dark:text-red-400' : 'text-amber-500'}`}>
              ...
              {rec.suggestedTimeDirection !== 'neutral' && (
                <div className="bg-white dark:bg-slate-800/60 rounded px-2 py-1.5 mb-1.5 border border-slate-200 dark:border-slate-700/50">
                  <div className="text-[7px] text-slate-400 dark:text-slate-500 uppercase tracking-wider font-semibold mb-1">Time suggestion</div>
                  <div className="grid grid-cols-[auto_1fr] gap-x-2 gap-y-0.5 text-[9px] items-center">
                    ...
                    <span className="text-slate-400 dark:text-slate-500">Direction:</span>
                    ...
                    <span className="text-slate-400 dark:text-slate-500">Target EC @:</span>
                    <span className="font-bold text-slate-700 dark:text-slate-300">
                    ...
                    <span className="text-slate-400 dark:text-slate-500">Delta:</span>
              ...
              <div className="flex flex-col gap-1.5 mt-1.5">
                {/* Grind direction bar */}
                <div className="flex items-center gap-2">
                  <span className="text-[7px] text-slate-400 dark:text-slate-500 w-10 text-right">Finer</span>
                  ...
                  <span className="text-[7px] text-slate-400 dark:text-slate-500 w-10">Coarser</span>
                </div>

                {/* Time direction bar — suggested brew time direction */}
                <div className="flex items-center gap-2">
                  <span className="text-[7px] text-slate-400 dark:text-slate-500 w-10 text-right">Shorter</span>
                  <div className="flex-1 h-3 relative">
                    <div className="absolute inset-0 rounded-full overflow-hidden flex">
                  ...
                  <span className="text-[7px] text-slate-400 dark:text-slate-500 w-10">Longer</span>
                </div>
              </div>

              <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1 pt-1 border-t border-slate-200 dark:border-slate-700/50">
                <span className="text-[9px] text-slate-400 dark:text-slate-500">
                  Suggested finish EC: <strong className="text-slate-700 dark:text-slate-300">{rec.finishEC.toFixed(1)}</strong>
                </span>
                {hasTargetEC && (
                  <span className="text-[9px] text-slate-400 dark:text-slate-500">
                    Your target: <strong className="text-slate-700 dark:text-slate-300">{parsedTargetEC.toFixed(1)}</strong>
                    <span className={`ml-1 ${Math.abs(parsedTargetEC - rec.finishEC) < 1 ? 'text-emerald-600' : 'text-amber-600 dark:text-amber-400'}`}>
                ...
                {analysis.ecAtTargetTime !== null && (
                  <span className="text-[9px] text-slate-400 dark:text-slate-500">
                    ...
                    <strong className="text-slate-700 dark:text-slate-300"> {analysis.ecAtTargetTime.toFixed(1)}</strong>
                  </span>
                )}
              </div>
            </div>
