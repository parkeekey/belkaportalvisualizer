...
function grindAdvice(drainRate: number, finesPct: number, grindSetting: number): string {
  if (drainRate < 0.4) return `Go coarser 2-3 clicks (#${Math.min(40, grindSetting + 3)}) — drain is too slow, fines are clogging the bed`;
  if (drainRate < 0.7) return `Go coarser 1 click (#${Math.min(40, grindSetting + 1)}) — drain is sluggish`;
  if (drainRate > 4) return `Go finer 2-3 clicks (#${Math.max(1, grindSetting - 3)}) — water is flushing through too quickly`;
  if (drainRate > 2.5) return `Go finer 1 click (#${Math.max(1, grindSetting - 1)}) — flow is too fast`;
  if (finesPct > 15) return `Fines are high (${finesPct.toFixed(0)}%) — try a cleaner burr profile or tighter distribution`;
  return 'Grind is in the sweet spot for this dose and pour';
}

function ratioAdvice(ratio: number): string {
  if (ratio < 13) return 'Increase ratio to 1:16+ — tight ratios are hard to control EY and drain stability';
  if (ratio < 14) return 'Consider 1:15-1:16 for a more forgiving TDS/EY balance';
  if (ratio > 19) return 'Decrease ratio to 1:16 — dilute brews risk low TDS and weak body';
...
function turbAdvice(pourRate: number, pourHeightCm: number, channelRisk: number): string {
  ...
  if (channelRisk > 0.45) return 'Reduce pour height and/or flow rate — channel risk is too high, causing uneven extraction';
  if (pourHeightCm < 4) return 'Pour from 5-8 cm height for better bed agitation without channeling';
  if (pourHeightCm > 10) return 'Lower pour height to 5-8 cm — high pours increase channel risk';
  return 'Pour rate and height are well balanced for extraction';
}

function tempAdvice(waterTempC: number): string {
  if (waterTempC < 80) return 'Increase water temp to 92-96°C — solubility is too low for effective extraction';
  if (waterTempC < 88) return 'Bump temp to 93°C for better solubility and extraction yield';
  if (waterTempC > 96) return 'Cool water to 93-96°C — near-boiling risks bitterness and over-extraction';
...
export default function FoundationEfficiency({
  ...
  const bars: { key: AdviceKey; label: string; score: number; desc: string }[] = [
    { key: 'grind', label: 'Grind', score: grindScore, desc: drainRate < 0.4 ? 'Too fine — clogging' : drainRate > 2.5 ? 'Too coarse — fast flow' : `${drainRate.toFixed(1)} g/s drain` },
    { key: 'ratio', label: 'Ratio', score: ratioScore, desc: ratio < 13 ? 'Concentrated — hard to control EY' : ratio > 19 ? 'Dilute — low TDS risk' : `${ratio}:1 — balanced range` },
    { key: 'turb', label: 'Turbulence', score: turbScore, desc: pourRate < 4.5 ? 'Low agitation — weak extraction' : channelRisk > 0.45 ? 'High channel risk — too aggressive' : 'Good agitation for extraction' },
    { key: 'temp', label: 'Temp', score: tempScore, desc: waterTempC < 85 ? 'Low solubility — under-extraction risk' : waterTempC > 96 ? 'Near boiling — bitterness risk' : `${waterTempC}°C — good solubility` },
  ];
