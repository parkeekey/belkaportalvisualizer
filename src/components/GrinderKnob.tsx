...
const GrinderKnob: React.FC<GrinderKnobProps> = ({ grinderName, onGrinderNameChange, grindSize, onGrindSizeChange, micron, expectedTDSMin, expectedTDSMax }) => {
  ...
  return (
    <div className="border border-emerald-200 dark:border-emerald-800 dark:border-emerald-800 rounded-lg bg-emerald-50 dark:bg-emerald-900/20/50 dark:bg-emerald-900/10 p-3 space-y-2">
      <div className="flex items-center gap-2 text-xs flex-wrap">
        <label className="text-slate-400 dark:text-slate-500 dark:text-slate-500">Grinder:</label>
        <input type="text" value={grinderName} onChange={(e) => onGrinderNameChange(e.target.value)} placeholder="e.g. Ode Gen 2" className="w-20 px-1.5 py-0.5 text-xs border border-emerald-300 dark:border-emerald-700 dark:border-emerald-700 rounded focus:outline-none focus:ring-2 focus:ring-emerald-400" />
        <span className="text-slate-300 dark:text-slate-600 dark:text-slate-600">|</span>
        <label className="text-slate-400 dark:text-slate-500 dark:text-slate-500">Total clicks:</label>
        <input type="number" value={totalClicks || ''} max={100} onChange={(e) => setTotalClicks(Math.min(100, Math.max(0, parseInt(e.target.value) || 0)))} className="w-12 px-1 py-0.5 text-xs border border-emerald-300 dark:border-emerald-700 dark:border-emerald-700 rounded text-center focus:outline-none focus:ring-2 focus:ring-emerald-400" placeholder="0" />
        <span className="text-slate-300 dark:text-slate-600 dark:text-slate-600">|</span>
        <label className="text-slate-400 dark:text-slate-500 dark:text-slate-500">Micro:</label>
        <input type="number" min={1} max={10} step={1} value={microStep} onChange={(e) => setMicroStep(Math.max(1, Math.min(10, parseInt(e.target.value) || 1)))} className="w-10 px-1 py-0.5 text-xs border border-emerald-300 dark:border-emerald-700 dark:border-emerald-700 rounded text-center focus:outline-none focus:ring-2 focus:ring-emerald-400" />
        <span className="text-[10px] text-slate-400 dark:text-slate-500 dark:text-slate-500">(step {step})</span>
        <span className="text-slate-300 dark:text-slate-600 dark:text-slate-600">|</span>
        <label className="flex items-center gap-1 cursor-pointer">
          <input type="checkbox" checked={isElectric} onChange={(e) => setIsElectric(e.target.checked)} className="rounded" />
          <span className="text-slate-400 dark:text-slate-500 dark:text-slate-500">Electric</span>
        </label>
      </div>
      ...
      <div className="flex items-start gap-3 flex-wrap">
        <div ref={dialRef} className="relative flex-shrink-0" style={{ width: outerPx, height: outerPx, touchAction: 'none' }} onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp}>
          ...
          <div className="absolute rounded-full border-2 border-emerald-400 bg-white dark:bg-slate-800 dark:bg-slate-200 shadow-sm"
            style={{
        ...
        <div className="flex flex-col gap-2 min-w-[180px] flex-1">
          <div className="inline-flex items-center self-start border border-emerald-300 dark:border-emerald-700 dark:border-emerald-700 rounded-lg bg-white dark:bg-slate-800 dark:bg-slate-800 shadow-sm overflow-hidden">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            <div className="text-[10px] text-slate-400 dark:text-slate-500 dark:text-slate-500 tabular-nums">#{grindSize > 0 ? grindSize : '—'}  |  {micron > 0 ? `${micron}µm` : '—'}</div>
            ...
            <button onClick={() => setShowCalc(v => !v)} className={`px-2.5 py-2 rounded text-xs font-bold border ${showCalc ? 'bg-emerald-200 text-emerald-800 dark:text-emerald-200 border-emerald-400 dark:bg-emerald-800 dark:text-emerald-200 dark:border-emerald-600' : 'bg-white dark:bg-slate-800 dark:bg-slate-800 text-slate-500 dark:text-slate-400 dark:text-slate-400 border-slate-300 dark:border-slate-600 dark:border-slate-600 hover:bg-slate-100 dark:hover:bg-slate-700 dark:hover:bg-slate-700'}`}>±</button>
            <button onClick={() => { setFocusMode(v => !v); if (!focusMode) { setFocusFrom(Math.max(0, Math.floor(clicks) - 1)); setFocusTo(Math.min(totalClicks, Math.ceil(clicks) + 1)); } }} className={`px-2.5 py-2 rounded text-xs font-bold border ${focusMode ? 'bg-amber-200 text-amber-800 dark:text-amber-200 border-amber-400 dark:bg-amber-800 dark:text-amber-200 dark:border-amber-600' : 'bg-white dark:bg-slate-800 dark:bg-slate-800 text-slate-500 dark:text-slate-400 dark:text-slate-400 border-slate-300 dark:border-slate-600 dark:border-slate-600 hover:bg-slate-100 dark:hover:bg-slate-700 dark:hover:bg-slate-700'}`}>🔍</button>
          </div>
          {showCalc && (
            <div className="flex items-center gap-2 text-[10px] bg-white dark:bg-slate-800 dark:bg-slate-800 border border-emerald-200 dark:border-emerald-800 dark:border-emerald-800 rounded-lg px-3 py-2 shadow-sm flex-wrap">
              <div className="flex items-center gap-1">
                <label className="text-slate-400 dark:text-slate-500 dark:text-slate-500">#grind1</label>
                <input type="number" value={calcLow} onChange={(e) => setCalcLow(parseFloat(e.target.value) || 0)} className="w-12 px-1 py-0.5 text-xs border border-slate-200 dark:border-slate-700 dark:border-slate-700 rounded text-center" />
              </div>
              <div className="flex items-center gap-1">
                <label className="text-slate-400 dark:text-slate-500 dark:text-slate-500">#grind2</label>
                <input type="number" value={calcHigh} onChange={(e) => setCalcHigh(parseFloat(e.target.value) || 0)} className="w-12 px-1 py-0.5 text-xs border border-slate-200 dark:border-slate-700 dark:border-slate-700 rounded text-center" />
              </div>
              <span className="text-slate-300 dark:text-slate-600 dark:text-slate-600">|</span>
              <label className="flex items-center gap-1 cursor-pointer">
                <input type="radio" name="calcDir" checked={calcDir === 'coarser'} onChange={() => setCalcDir('coarser')} className="rounded-full" />
                <span className="text-slate-500 dark:text-slate-400 dark:text-slate-400">% coarser</span>
              </label>
              <label className="flex items-center gap-1 cursor-pointer">
                <input type="radio" name="calcDir" checked={calcDir === 'finer'} onChange={() => setCalcDir('finer')} className="rounded-full" />
                <span className="text-slate-500 dark:text-slate-400 dark:text-slate-400">% finer</span>
              </label>
              <div className="flex items-center gap-1">
                <input type="number" min={0} max={100} value={calcPct} onChange={(e) => setCalcPct(Math.max(0, Math.min(100, parseFloat(e.target.value) || 0)))} className="w-12 px-1 py-0.5 text-xs border border-slate-200 dark:border-slate-700 dark:border-slate-700 rounded text-center" />
              ...
              <span className="font-bold text-emerald-700 dark:text-emerald-400 dark:text-emerald-400 tabular-nums">
          ...
          <div className="flex items-center gap-2 text-[10px] flex-wrap">
            <label className="text-slate-400 dark:text-slate-500 dark:text-slate-500">Turn Speed</label>
            ...
            <span className="text-slate-500 dark:text-slate-400 dark:text-slate-400 tabular-nums w-5">{(turnSpeed * 10000).toFixed(0)}</span>
          </div>
          <div className="flex items-center gap-1">
            <button onClick={() => persistedSetZoom(z => Math.max(0.5, z - 0.25))} disabled={zoomLocked} className="w-5 h-5 rounded border border-slate-300 dark:border-slate-600 dark:border-slate-600 bg-white dark:bg-slate-800 dark:bg-slate-800 text-slate-500 dark:text-slate-400 dark:text-slate-400 text-[9px] font-bold hover:bg-slate-100 dark:hover:bg-slate-700 dark:hover:bg-slate-700 flex items-center justify-center disabled:opacity-30 disabled:cursor-not-allowed">−</button>
            <span className="text-[9px] text-slate-400 dark:text-slate-500 dark:text-slate-500 tabular-nums w-6 text-center">{(zoom * 100).toFixed(0)}%</span>
            <button onClick={() => persistedSetZoom(z => Math.min(3, z + 0.25))} disabled={zoomLocked} className="w-5 h-5 rounded border border-slate-300 dark:border-slate-600 dark:border-slate-600 bg-white dark:bg-slate-800 dark:bg-slate-800 text-slate-500 dark:text-slate-400 dark:text-slate-400 text-[9px] font-bold hover:bg-slate-100 dark:hover:bg-slate-700 dark:hover:bg-slate-700 flex items-center justify-center disabled:opacity-30 disabled:cursor-not-allowed">+</button>
            <button onClick={() => { const next = !zoomLocked; setZoomLocked(next); localStorage.setItem('belkaKnobZoomLocked', String(next)); }} className={`w-5 h-5 rounded border text-[9px] font-bold flex items-center justify-center transition-all ${zoomLocked ? 'bg-emerald-100 dark:bg-emerald-900/30 border-emerald-300 dark:border-emerald-700 text-emerald-700 dark:text-emerald-400 dark:bg-emerald-900 dark:border-emerald-700 dark:text-emerald-300' : 'bg-white dark:bg-slate-800 dark:bg-slate-800 border-slate-300 dark:border-slate-600 dark:border-slate-600 text-slate-400 dark:text-slate-500 dark:text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800 dark:bg-slate-900/50 dark:hover:bg-slate-700'}`} title={zoomLocked ? 'Zoom locked' : 'Lock zoom'}>
              {zoomLocked ? '🔒' : '🔓'}
            </button>
          </div>
          {focusMode && (
            <div className="flex items-center gap-2 text-[10px] bg-amber-50 dark:bg-amber-900/20 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 dark:border-amber-800 rounded-lg px-3 py-2 flex-wrap">
              <span className="text-amber-600 dark:text-amber-400 dark:text-amber-400 font-semibold">Focus</span>
              <span className="text-slate-300 dark:text-slate-600 dark:text-slate-600">|</span>
              <label className="text-slate-400 dark:text-slate-500 dark:text-slate-500">From</label>
              ...
              <span className="text-slate-300 dark:text-slate-600 dark:text-slate-600">→</span>
              <label className="text-slate-400 dark:text-slate-500 dark:text-slate-500">To</label>
      ...
      {showHistory && history.length > 0 && (
        <div className="max-h-48 overflow-y-auto space-y-1 border-t border-emerald-200 dark:border-emerald-800 dark:border-emerald-800 pt-2">
          {history.map((e) => (
            editingId === e.id ? (
              <div key={e.id} className="flex flex-col gap-1 text-[10px] bg-white dark:bg-slate-800 dark:bg-slate-800 rounded px-2 py-1.5 border border-emerald-300 dark:border-emerald-700 dark:border-emerald-700">
                <div className="flex items-center gap-1 flex-wrap">
                  ...
                  <span className="text-slate-400 dark:text-slate-500 dark:text-slate-500">#</span>
                  <input type="number" step={step} value={e.clicks} onChange={(v) => updateEntry(e.id, { clicks: parseFloat(parseFloat(v.target.value).toFixed(decimals)) || 0 })} className="w-12 px-1 py-0.5 text-xs border border-slate-200 dark:border-slate-700 dark:border-slate-700 rounded text-center" />
                  <span className="text-slate-400 dark:text-slate-500 dark:text-slate-500">µm</span>
                  <input type="number" step={1} value={e.micron} onChange={(v) => updateEntry(e.id, { micron: parseFloat(v.target.value) || 0 })} className="w-12 px-1 py-0.5 text-xs border border-slate-200 dark:border-slate-700 dark:border-slate-700 rounded text-center" />
                  <span className="text-slate-300 dark:text-slate-600 dark:text-slate-600">|</span>
                  <span className="text-slate-400 dark:text-slate-500 dark:text-slate-500">TDS</span>
                  <input type="number" step="0.01" value={e.expectedTDSMin} onChange={(v) => updateEntry(e.id, { expectedTDSMin: parseFloat(v.target.value) || 0 })} className="w-12 px-1 py-0.5 text-xs border border-slate-200 dark:border-slate-700 dark:border-slate-700 rounded text-center" />
                  <span className="text-slate-300 dark:text-slate-600 dark:text-slate-600">~</span>
                  <input type="number" step="0.01" value={e.expectedTDSMax} onChange={(v) => updateEntry(e.id, { expectedTDSMax: parseFloat(v.target.value) || 0 })} className="w-12 px-1 py-0.5 text-xs border border-slate-200 dark:border-slate-700 dark:border-slate-700 rounded text-center" />
              ...
              <div key={e.id} className={`flex items-center gap-1.5 text-[10px] rounded px-2 py-1 border ${newEntryId === e.id ? 'border-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 dark:bg-emerald-900/20 shadow-sm' : 'border-slate-100 dark:border-slate-700 dark:border-slate-700 bg-white dark:bg-slate-800 dark:bg-slate-800'}`}>
                ...
                <span className={`font-medium tabular-nums ${e.like ? 'text-green-600 dark:text-green-400 dark:text-green-400' : e.dislike ? 'text-red-500 dark:text-red-400 dark:text-red-400' : 'text-slate-500 dark:text-slate-400 dark:text-slate-400'}`}>#{e.clicks}</span>
                {e.micron > 0 && <span className="tabular-nums text-slate-400 dark:text-slate-500 dark:text-slate-500">{e.micron}µm</span>}
                {e.rpm > 0 && <span className="tabular-nums text-slate-400 dark:text-slate-500 dark:text-slate-500">{e.rpm}rpm</span>}
                <span className="text-slate-300 dark:text-slate-600 dark:text-slate-600">|</span>
                <span className={`tabular-nums ${e.tdsIsPlan ? 'text-amber-600 dark:text-amber-400 dark:text-amber-400' : 'text-slate-500 dark:text-slate-400 dark:text-slate-400'}`}>{e.expectedTDSMin > 0 ? e.expectedTDSMin.toFixed(2) : '?'}–{e.expectedTDSMax > 0 ? e.expectedTDSMax.toFixed(2) : '?'}%{e.tdsIsPlan ? ' (plan)' : ' (manual)'}</span>
                <span className="ml-auto text-slate-300 dark:text-slate-600 dark:text-slate-600">{e.date}</span>
                <button onClick={() => setEditingId(e.id)} className="text-slate-400 dark:text-slate-500 dark:text-slate-500 hover:text-slate-600 dark:text-slate-400 dark:hover:text-slate-300 dark:text-slate-600 px-0.5">✎</button>
