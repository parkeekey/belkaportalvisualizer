    ...
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-5 select-none">
      ...
      <div className="flex items-center gap-3 max-w-xs mx-auto">
        <div className="flex-1">
          ...
          <input type="number" min={0} max={40} step={1} value={grindMin}
            ...
            className="w-full text-center text-sm font-bold text-blue-800 bg-white border border-blue-200 rounded-lg py-2" />
      ...
      <div className="max-w-xs mx-auto w-full">
        <div className="flex items-center justify-between mb-1">
          <span className="text-[9px] text-slate-400 font-semibold">Particle size distribution</span>
          ...
          {micronSizes.map((m, i) => {
            ...
            return (
              <div key={m} className={`flex flex-col items-center gap-0.5 shrink-0 ${isFines ? 'opacity-60' : ''} ${mode === 'manual' ? 'cursor-pointer' : ''}`}
                ...
                />}
                <span className={`text-[6px] font-mono ${isFines ? 'text-red-400' : isCenter ? 'text-blue-700 font-bold' : inRange ? 'text-blue-500' : 'text-slate-300'}`}>
                  {m}
                </span>
        ...
        <div className="flex items-center justify-between mt-1 flex-wrap gap-x-2">
            <span className="text-[8px] text-slate-500">Surface: <strong className="text-blue-700">{surfaceArea.toFixed(0)} cm²</strong></span>
            <span className="text-[8px] text-slate-500">Fines &lt;200µm: <strong className="text-red-500">
            ...
            {mode === 'simulate' ? (
              <div className="flex items-center gap-1">
                ...
                <button onClick={() => setSiftSeed(prev => prev + 1)}
                  className="text-[8px] font-bold text-amber-700 bg-amber-50 border border-amber-200 rounded px-2 py-0.5 hover:bg-amber-100">
                  ⟳ Sift
                </button>
              </div>
            ) : (
              <button onClick={() => setManualRaw(micronSizes.map(() => 5))}
                className="text-[8px] font-bold text-slate-600 bg-slate-100 border border-slate-300 rounded px-2 py-0.5 hover:bg-slate-200">
                ↻ Reset taps
              </button>
            )}
        </div>
        {mode === 'manual' && (
          <div className="mt-1.5 flex flex-col items-center gap-1">
            <div className="flex items-center gap-1">
              <span className="text-[7px] text-slate-400 mr-0.5">Tap:</span>
            ...
            <div className="text-[7px] text-slate-400 italic text-center">
        ...
        {mode === 'simulate' && calibrated && (
          <div className="text-[7px] text-slate-400 italic mt-0.5 text-center">
      ...
      <div className="max-w-xs mx-auto w-full">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="flex-1 min-w-[60px]">
            <span className="text-[7px] text-slate-400 font-semibold">Roast</span>
            <select value={roast} onChange={e => setRoast(e.target.value as Roast)}
              className="w-full text-[10px] font-bold text-slate-700 bg-white border border-slate-200 rounded py-1.5 px-1">
          ...
          {/* Process */}
          <div className="flex-1 min-w-[60px]">
            <span className="text-[7px] text-slate-400 font-semibold">Process</span>
          ...
          <div className="flex-1 min-w-[80px]">
            <span className="text-[7px] text-slate-400 font-semibold">Burr</span>
            ...
            <div className="text-[6px] text-slate-400 mt-0.5 leading-tight">{burrProfile.desc}</div>
          </div>
          {/* Humidity */}
          <div className="flex-[2] min-w-[80px]">
            <div className="flex items-center justify-between text-[7px] mb-0.5">
              <span className="text-slate-400 font-semibold">Humidity</span>
          ...
          <div className="flex-[2] min-w-[80px]">
            <div className="flex items-center justify-between text-[7px] mb-0.5">
              <span className="text-slate-400 font-semibold">Water Temp</span>
              <span className="font-bold text-slate-600">{waterTempC}C</span>
            </div>
      ...
      <div className="max-w-xs mx-auto w-full">
        <div className="flex items-center justify-between text-[7px] mb-0.5">
          <span className="text-slate-400 font-semibold">Burr Ø{dia}mm</span>
          <span className="font-bold text-slate-600">{dia >= 80 ? 'High inertia' : dia >= 60 ? 'Medium inertia' : 'Light spin'}</span>
        </div>
        ...
        <div className="flex justify-between text-[6px] text-slate-300 mt-px">
      ...
      <div className="flex items-center gap-3 max-w-xs mx-auto">
        <div className="flex-1">
          <span className="text-[8px] text-slate-500 font-semibold">Dose (g)</span>
          <input type="number" min={1} max={60} step={0.5} value={dose}
            onChange={e => { const v = parseFloat(e.target.value); if (!isNaN(v) && v > 0) setDose(v); }}
            className="w-full text-center text-sm font-bold text-slate-700 bg-white border border-slate-200 rounded-lg py-2" />
        </div>
        <div className="flex-1">
          <span className="text-[8px] text-slate-500 font-semibold">Target RPM</span>
      ...
      <div className="flex flex-col items-center">
        ...
        >
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-16 h-16 rounded-full bg-white border-2 border-blue-300 flex items-center justify-center shadow-inner">
        ...
        <div className="w-full max-w-xs mt-4">
          <div className="flex items-center justify-between text-[9px] mb-0.5">
            <span className="text-slate-400 font-semibold">Grind progress</span>
        ...
        <div className="w-full max-w-xs mt-2 text-center">
          <span className="text-[8px] text-slate-400">
            Density: <strong className={kValue >= 60 ? 'text-blue-600' : kValue >= 35 ? 'text-slate-600' : 'text-amber-600'}>{kValue}</strong>
            {' · '}
        ...
        <div className="flex items-center gap-3 mt-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] text-slate-400 tabular-nums w-16">
          ...
          <button onClick={resetGrind}
            className="px-3 py-1.5 text-[10px] font-bold bg-slate-200 text-slate-600 rounded-lg border border-slate-300 hover:bg-slate-300">
            ↻ Reset
          </button>
      ...
      {logs.length > 0 && (
        <section className="max-w-xs mx-auto w-full">
          <div className="flex items-center justify-between mb-1">
            <h3 className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Grind Log</h3>
            <button onClick={() => setLogs([])}
              className="text-[8px] text-red-400 hover:text-red-600">Clear</button>
          </div>
          <div className="space-y-1 max-h-40 overflow-y-auto">
            {logs.map((l, i) => (
              <div key={i} className="text-[9px] text-slate-500 bg-slate-50 px-2 py-1.5 rounded border border-slate-200 flex items-center gap-2 flex-wrap">
      ...
      <Brew dose={dose} grindSetting={grindSetting} micronSetting={micronSetting} finesPct={finesPct} surfaceArea={surfaceArea}
        roast={roast} process={process} humidity={humidity} waterTempC={waterTempC} burrProfileName={burrProfile.name} />
    </div>
  );
}
