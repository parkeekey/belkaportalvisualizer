...
export default function RecipeGenerator() {
  ...
  const cardColors: Record<string, { border: string; bg: string; label: string; value: string }> = {
    dose:   { border: 'border-l-slate-400', bg: 'bg-slate-50 dark:bg-slate-900/50',     label: 'text-slate-500 dark:text-slate-400', value: 'text-slate-800 dark:text-white' },
      ...
      <header className="sticky top-0 z-10 bg-white dark:bg-slate-800/90 backdrop-blur-sm border-b border-slate-200 dark:border-slate-700">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <button onClick={() => { setSelectedProfileId(''); }}
            className="px-2 py-1 text-[10px] font-semibold border border-slate-200 dark:border-slate-700 rounded-lg text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 dark:hover:bg-slate-700"
          >✕ Clear</button>
        </div>
      </header>
          ...
          <>
            ...
            {recipe && (
              <div className="bg-white dark:bg-slate-800 rounded-xl border border-amber-200 dark:border-amber-800 shadow-sm p-4">
                ...
                {/* Three-pillar ratio emphasis */}
                {(() => {
                  const r = Math.max(14, Math.min(18, recipe.ratio));
                  const aroma = Math.round(Math.max(5, 100 - (r - 14) * 22));
                  const mouthfeel = Math.round(Math.max(5, (r - 14) * 22));
                  const distFromIdeal = Math.abs(r - 16);
                  const flavor = Math.round(Math.max(5, 100 - distFromIdeal * 30));
                  const peak = aroma >= flavor && aroma >= mouthfeel ? 'aroma' : flavor >= aroma && flavor >= mouthfeel ? 'flavor' : 'mouthfeel';
                  const emphasis = peak === 'aroma' ? 'bright, floral, tea-like clarity' : peak === 'flavor' ? 'balanced sweetness, complexity, and structure' : 'heavy body, chocolate, and richness';
                  const isAroma = aggregate.dimension === 'aroma';
                  const isFlavor = aggregate.dimension === 'flavor';
                  const isMouthfeel = aggregate.dimension === 'mouthfeel';
                  return (
                    <div className="mb-4">
                      <div className="text-[9px] font-semibold text-slate-500 dark:text-slate-400 mb-1.5">Ratio emphasis</div>
                      <div className="flex gap-3">
                        <div className="flex-1 flex flex-col items-center">
                          <div className="text-[7px] font-semibold mb-0.5 text-rose-600 dark:text-rose-400">Aroma</div>
                          <div className={`h-16 w-full rounded-lg overflow-hidden flex flex-col-reverse ${isAroma ? 'bg-rose-100 dark:bg-rose-900/30 ring-1 ring-rose-400' : 'bg-rose-50 dark:bg-rose-900/10'}`}>
                            <div className={`transition-all ${isAroma ? 'bg-rose-400 dark:bg-rose-500' : 'bg-rose-300 dark:bg-rose-700'}`} style={{ height: `${aroma}%` }} />
                          </div>
                          <div className="text-[10px] font-bold mt-0.5 text-rose-700 dark:text-rose-300">{aroma}%</div>
                        </div>
                        <div className="flex-1 flex flex-col items-center">
                          <div className="text-[7px] font-semibold mb-0.5 text-amber-600 dark:text-amber-400">Flavor</div>
                          <div className={`h-16 w-full rounded-lg overflow-hidden flex flex-col-reverse ${isFlavor ? 'bg-amber-100 dark:bg-amber-900/30 ring-1 ring-amber-400' : 'bg-amber-50 dark:bg-amber-900/10'}`}>
                            <div className={`transition-all ${isFlavor ? 'bg-amber-400 dark:bg-amber-500' : 'bg-amber-300 dark:bg-amber-700'}`} style={{ height: `${flavor}%` }} />
                          </div>
                          <div className="text-[10px] font-bold mt-0.5 text-amber-700 dark:text-amber-300">{flavor}%</div>
                        </div>
                        <div className="flex-1 flex flex-col items-center">
                          <div className="text-[7px] font-semibold mb-0.5 text-orange-600 dark:text-orange-400">Mouthfeel</div>
                          <div className={`h-16 w-full rounded-lg overflow-hidden flex flex-col-reverse ${isMouthfeel ? 'bg-orange-100 dark:bg-orange-900/30 ring-1 ring-orange-400' : 'bg-orange-50 dark:bg-orange-900/10'}`}>
                            <div className={`transition-all ${isMouthfeel ? 'bg-orange-400 dark:bg-orange-500' : 'bg-orange-300 dark:bg-orange-700'}`} style={{ height: `${mouthfeel}%` }} />
                          </div>
                          <div className="text-[10px] font-bold mt-0.5 text-orange-700 dark:text-orange-300">{mouthfeel}%</div>
                        </div>
                      </div>
                      <div className="text-[8px] text-slate-400 dark:text-slate-500 mt-1 text-center">
                        Ratio 1:{recipe.ratio} <span className="font-semibold text-slate-500 dark:text-slate-400">{aggregate.dimension === peak ? 'matches' : 'leans'} {emphasis}</span>
                      </div>
                    </div>
                  );
                })()}
